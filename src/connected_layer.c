...
void backward_connected_layer(layer l, matrix prev_delta)
{
    ...
    // l.dw = l.dw - dL/dw
    matrix in_trans = transpose_matrix(in);
    matrix dl_dw  = matmul(in_trans, delta);
    axpy_matrix(-1, dl_dw, l.dw);
    free_matrix(dl_dw);
    free_matrix(in_trans);
    ...
    if(prev_delta.data){
        ...
        matrix lw_trans = transpose_matrix(l.w);
        matrix dl_din = matmul(delta, lw_trans);
        axpy_matrix(1, dl_din, prev_delta);
        free_matrix(dl_din);
        free_matrix(lw_trans);
    }
}
